...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,top.riverelder.arkham";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("top.riverelder.arkham", 1, 9, "来自阿卡姆的骰娘", "1.2.4", 16, "RiverElder", "一只来自阿卡姆的跑团小助手", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("top.riverelder.arkham", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("top.riverelder.arkham", AppData.CQLog);	
